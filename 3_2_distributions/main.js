...
/* LOAD DATA */
d3.csv("../data/stateCapitals.csv", d3.autoType).then((raw_data) => {
  // + SET YOUR DATA PATH
...
function init() {
  ...
  xScale = d3
    .scaleLinear()
    .domain(d3.extent(state.data, (d) => d.latitude))
    .range([margin.left, width - margin.right]);

  yScale = d3
    .scaleLinear()
    .domain(d3.extent(state.data, (d) => d.longtitude))
  ...
  selectElement
    .selectAll("option")
    .data([
      // can do this programmatically also if we want
...
function draw() {
  ...
  const filteredData = state.data.filter(
    (d) => state.selectedParty === "All" || state.selectedParty === d.Party
  );
          ...
          .call(
            (sel) =>
              sel
                .transition()
                .duration(500)
                .attr("cx", (d) => xScale(d.longtitude)) // transition to correct position
          ),
